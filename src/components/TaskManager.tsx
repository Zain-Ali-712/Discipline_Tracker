...
const TaskManager: React.FC<TaskManagerProps> = ({
  ...
  const isOutreachExcluded = outreachTask?.isExcluded ? outreachTask.isExcluded(new Date(date)) : isWeekend;
  ...
  return (
    <div className={`rounded-3xl border-2 shadow-2xl transition-colors duration-300 ${theme === 'dark'
      ...
      <div className={`p-6 border-b ${theme === 'dark'
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex flex-col items-end gap-3">
            <div className={`text-3xl font-bold ${progress >= 80
              ? 'text-emerald-400'
              : progress >= 60
                ? 'text-amber-400'
                : 'text-rose-400'
            ...
            <button
              onClick={onSave}
              disabled={isSaved}
              className={`px-6 py-2.5 text-base rounded-xl font-bold transition-all flex items-center gap-2 ${isSaved
                ? theme === 'dark'
        ...
        {/* Progress Bar */}
        <div className="mt-5">
          <div className={`h-3 rounded-full overflow-hidden ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-300'
            }`}>
            <div
          ...
          <div className={`flex justify-between text-base mt-2 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'
            }`}>
            <span>0%</span>
      ...
      <div className="p-6">
        ...
        <div className="space-y-4">
          {tasks.map(task => {
            ...
            return (
              <div
                key={task.id}
                className={`rounded-xl border-2 transition-all ${task.completed || (task.id === 'outreach' && isOutreachExcluded)
                  ? theme === 'dark'
              ...
              >
                <div className="p-4">
                    ...
                    <div className="flex items-center gap-2">
                      {(task.id === 'project' || task.id === 'advance-project') && (
                        <button
                          ...
                          className={`w-8 h-8 rounded-lg flex items-center justify-center ${theme === 'dark'
                      ...
                      <button
                        ...
                        disabled={isDisabled}
                        className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${task.completed || (task.id === 'outreach' && isOutreachExcluded)
                      ...
                      >
                        {task.completed || (task.id === 'outreach' && isOutreachExcluded) ? (
                          <FiCheck className="w-5 h-5" />
                        ) : (
                  ...
                  {isExpanded && (task.id === 'project' || task.id === 'advance-project') && (
                    <div className={`mt-4 pt-4 border-t ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'
                      ...
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className={`text-xl font-bold ${task.id === 'project'
                          ? projectHours >= 2
                            ? 'text-emerald-400'
                            : projectHours >= 1
                              ? 'text-amber-400'
                      ...
                      <div className="flex justify-center gap-2">
                        {[0, 1, 2, 3, 4].map(hour => (
                          <button
                            ...
                            disabled={isSaved}
                            className={`px-4 py-2.5 rounded-lg text-base transition-all ${(task.id === 'project' ? projectHours === hour : advanceProjectHours === hour)
                              ? task.id === 'project'
                                ? theme === 'dark'
                  ...
                  {task.id === 'outreach' && isOutreachExcluded && (
                    <div className={`mt-3 p-2 rounded-lg text-sm ${theme === 'dark' ? 'bg-blue-900/20 text-blue-300' : 'bg-blue-50 text-blue-700'
                      }`}>
                      Weekend - Outreach task is auto-credited. Enjoy your break!
                    </div>
                  )}
                </div>
              </div>
        ...
        {/* Summary */}
        <div className={`mt-6 p-5 rounded-xl border-2 ${theme === 'dark'
          ? 'bg-gray-800/50 border-gray-700'
          : 'bg-gray-50 border-gray-200'
          }`}>
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center">
              <div className={`text-base font-bold mb-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'
                }`}>
                Tasks Completed
              </div>
